...
: GetSelectedWord-	\ hwnd buff -- buff len ; buff must be at least 256
  { hwnd buff | ptr start end len -- buff len }

  1024 allocate				\ get 1k buffer
  if  drop  buff 0  exit  then			\ return buff/0 on failure ; SFP005
  -> ptr  1024 ptr w!				\ set for EM_GETLINE

  hwnd EM_LINEFROMCHAR -1 0 SendMessageA		\ line of current selection
  hwnd EM_GETLINE rot ptr SendMessageA drop	\ ptr -> 0 terminated Line
  hwnd EM_GETSEL ^ start ^ end SendMessageA drop	\ start and end
  hwnd EM_LINEINDEX -1 0 SendMessageA            \ idx of start of line
  dup  ^ start DECR ^ end DECR			\ form offsets into line
  ptr end start /string
  0 max 255 min				\ protection ; SFP009
  bl skip -trailing 255 min -> len		\ -- addr ; of word
  buff len move					\ copy to o/p buffer
  buff len					\ -- buff len
...
: (neweditboxproc)-	\ hwnd msg wParam lParam -- stat
  ...
  case  2 pick
    ...
    WM_LBUTTONDBLCLK of	        \ -- hwnd msg wParam lParam
      3 pick >r
      do-oldeditboxproc-			\ stat -- ; R: hwnd --
      r@ text[ GetSelectedWord-		\ -- stat c-addr u
      r> op-handle @			\ -- stat c-addr u hwnd sid
      dup richedit-dblclkxt @		\ -- stat c-addr u hwnd sid xt
      catch if 4drop endif		\ -- stat ; default action is 4DROP ; SFP013
    endof
...
\    ['] 4drop sid richedit-dblclkxt !   \ default double click xt

\    sid re.ForeColor @ sid re.BackColor @ sid gen-handle @ ChangeTextColours
\    sid re.BackColor @ -1 <> if		\ set background
\      sid gen-handle @ EM_SETBKGNDCOLOR 0 sid re.BackColor @ SendMessageA drop
\    then
...
(*
      sid re.ForeColor @ -1 <>
      if  sid re.ForeColor @ hwnd ForeColourSelection  endif
      sid re.BackColor @ -1 <>
      if  sid re.BackColor @ hwnd BackColourSelection  endif
*)
